...
/** Convert user space memory address to bus address
 * @param allocs
 * @param virt_addr Virtual address to convert, must not be NULL.
 * @param size Number of bytes starting at virt_addr that should be valid.
 * @return Bus address of the memory or VCIOA_LOCATION_NONE if the address was invalid. */
static uint32_t vcioa_virt_to_bus(vcio_allocs* allocs, unsigned long virt_addr, unsigned long size)
{	vcio_alloc* ap = allocs->List;
	vcio_alloc* ape = ap + allocs->Count;
	unsigned long* mp;
	vcio_pr_debug("%s(%p, %lx, %lx)", __func__, allocs, virt_addr, size);
	do if (ap == ape)
		return VCIOA_LOCATION_NONE;
	while (ap->Location == VCIOA_LOCATION_NONE || !(mp = vcioa_find_mapping_range(ap, virt_addr, size)));
	vcio_pr_debug("%s: %lx", __func__, *mp);
	return ap->Location + (virt_addr - *mp);
}
	...
	switch (MINOR(f->f_inode->i_rdev))
	{
		...
		{	vcio_data* data = f->private_data;
			...
			switch (ioctl_num) {
				...
				case IOCTL_EXEC_QPU2:
				{	vcio_exec_qpu2 p;
					if (unlikely(copy_from_user(&p, (void*)ioctl_param, sizeof p)))
						goto mem_fault;
					vcio_pr_debug("IOCTL_EXEC_QPU2 %x, %p, %x, %x", p.num_qpus, p.control, p.noflush, p.timeout);

					if (!p.num_qpus)
						rc = -ENODATA;
					else if (p.num_qpus > 12)
						rc = -EINVAL;
					else
					{	dma_addr_t vc_control;
						void* control = dma_alloc_coherent(vcio_pdev, PAGE_SIZE, &vc_control, GFP_ATOMIC);
						if (unlikely(!control))
							rc = -ENOMEM;
						else
						{	vcio_exec_qpu_entry* dp = control;
							vcio_exec_qpu2_entry* sp = control;
							if (unlikely(copy_from_user(dp, p.control, sizeof(vcio_exec_qpu2_entry) * p.num_qpus)))
							{	dma_free_coherent(vcio_pdev, PAGE_SIZE, control, vc_control);
								goto mem_fault;
							}
							// convert pointers, do it in reverse to avoid aliasing
							dp += p.num_qpus - 1;
							sp += p.num_qpus - 1;
							for (dp += p.num_qpus - 1, sp += p.num_qpus - 1; ; --sp, --dp)
							{	dp->uniforms = vcioa_virt_to_bus(&data->Allocations, (unsigned long)sp->uniforms, sizeof *sp->uniforms);
								dp->code = vcioa_virt_to_bus(&data->Allocations, (unsigned long)sp->code, sizeof(uint64_t) * 3); // shortest valid program
								if (unlikely(dp->uniforms == VCIOA_LOCATION_NONE || dp->code == VCIOA_LOCATION_NONE))
								{	rc = -EACCES;
									break;
								}
								if (dp == control) // success!
								{	if (likely(!(rc = vcio_set_enabled(data, 1))))
									{	vcio_read_perf_count(data, -1);
										vcio_pr_debug("Exec: %x", vc_control);
										if (unlikely(ExecuteQpu(p.num_qpus, vc_control | 0x40000000, p.noflush, p.timeout)))
											rc = -ENOEXEC;
										vcio_read_perf_count(data, 1);
									}
									break;
